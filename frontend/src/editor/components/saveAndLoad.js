...
const saveScene = async (sceneObjects, sceneSettings, sceneName) => {
    const dataToSave = {
    ...
    for (const obj of sceneObjects) {
        ...
        if (obj.material) {
            ...
            objectData.material = {
                color: typeof mat.color === 'string' ? mat.color : (mat.color?.getHexString ? `#${mat.color.getHexString()}` : '#ffffff'), // Handle string colors
                emissive: typeof mat.emissive === 'string' ? mat.emissive : (mat.emissive?.getHexString ? `#${mat.emissive.getHexString()}` : '#000000'), // Handle string colors
    ...
    try {
                ...
                const response = await axios.post('http://localhost:5050/save',
                    { objects: dataToSave.objects, sceneSettings: dataToSave.sceneSettings, sceneName }, // Send the actual data
                    {
...
const loadScene = async (sceneId, setSceneObjects, setSceneSettings) => {
    try {
        const response = await axios.get(`http://localhost:5050/get-scene?sceneId=${sceneId}`, {
