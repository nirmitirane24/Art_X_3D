...
import { loadScene } from "./components/saveAndLoad"; // Import loadScene

const EditorManager = () => {
    const navigate = useNavigate();
    const [loading, setLoading] = useState(true);
    const [username, setUsername] = useState("");
    const [currentSceneId, setCurrentSceneId] = useState(null); // Track current scene ID
    const [currentSceneName, setCurrentSceneName] = useState(""); // Track the current scene name
    ...
    const addModel = (type) => {
        saveToUndoStack([...sceneObjects], { ...sceneSettings });
        // Reset currentSceneId when adding a new model (new scene)
    ...
    // --- CORRECTED useEffect for initial scene loading ---
    useEffect(() => {
        const urlParams = new URLSearchParams(window.location.search);
        const sceneId = urlParams.get('sceneId');
        if (sceneId) {
            // Directly call loadScene with the numeric sceneId
            loadScene(sceneId, setSceneObjects, setSceneSettings);
            setCurrentSceneId(sceneId); // Set the ID

            // Also, fetch the scene name
          ...
          return (
              <div className="editor-container" onClick={deselectAllObjects}>
                  <Toolbar
                      ...
                      sceneObjects={sceneObjects} // Pass sceneObjects
                      sceneSettings={sceneSettings} // Pass sceneSettings
                  />
                  <div className="main-area">
                      <div className="sidebar">
                          <HierarchyPanel
                              ...
                              currentSceneId={currentSceneId}
                              setCurrentSceneId={setCurrentSceneId}  // Pass setCurrentSceneId
                          />
                          <PropertiesPanel
                              selectedObjects={selectedObjects ?? []}
                              sceneObjects={sceneObjects ?? []}
